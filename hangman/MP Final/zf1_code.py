...
import pygame
import sys
# import button functionality from button for menu screen
...
# gets font for the meny screen
def get_font(size):  
    return pygame.font.Font("font.ttf", size)

# render the menu screen objects
def options():
    while True:
        GRAY = (182, 168, 168)
...
# starts after clicking on play button on menu screen-->
def play():
    ...
    used_letters_font = pygame.font.Font(None, font_size + 10)  # Increase font size for used letters
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_ESCAPE: 
                    ...
                    if letter in used_letters: #ignore letters used
                        continue
        ...
        win.blit(word_text, (80, 160))

        win.blit(hangman_imgs[hangman_step], (WIDTH - 550, 80))
        win.blit(lives_img[hangman_step], (WIDTH - 500, 10))

        for i, letter in enumerate(used_letters):
            text_color = GREEN if letter in word else RED  # Use different colors for correct and incorrect guesses
            letter_text = inkfree_font.render(letter, True, text_color)
            win.blit(letter_text, (20 + i * (font_size + 10), 240))
        ...
        if message == ("You Win! \n"
                       "Press Enter to REPLAY \n"
                        ...
                        "ESC to go back to the menu.\n"):
            wins += 1
            display_text(win, message, (20,300), font, BLUE)
        else:
           display_text(win, message, (20,300), font, RED)
           
        # Display wins count
        wins_text = font.render(f"Wins: {wins}", True, BLACK)
        win.blit(wins_text, (20, 550))

        # Display losses count
        losses_text = font.render(f"Losses: {losses}", True, BLACK)
        win.blit(losses_text, (20, 600))
